...
- **`src/main.js` (main process)** — creates the `BrowserWindow` (hidden/overlay titlebar), builds the native menu, owns OS integration: file save dialogs, ZIP export (JSZip), `nativeTheme` change events, and reading `monacorc.json` from disk. Holds all Node/`fs` access.
...
- **IndexedDB** (`SainteDevoteDB`) — tab content and metadata. Two object stores: `tabs` (id, title, order) and `content` (keyed by `tabId`). Content is saved on every `onDidChangeModelContent`. See `openDatabase` / `save*IndexedDB` / `load*IndexedDB` functions.
